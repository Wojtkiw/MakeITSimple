...
- "skladniki": CZĘŚCI MECHANIZMU lub kroki działania pojęcia — jak ono działa (np. dla spadku gradientowego: gradient, aktualizacja parametrów, iteracja; dla lasu losowego: drzewa decyzyjne, losowy wybór cech, głosowanie). Wypisz je KOMPLETNIE, po kolei, nie ucinaj do jednego-dwóch. W polu "opis" DODAJ informację (co to robi / po co), nie powtarzaj samej nazwy. Jeśli pojęcie jest czysto opisowe i nie ma naturalnych części — zostaw [].
